...
describe("instrument()", () => {
  ...
  it("emits TOOL_CALL_BLOCK metadata and carries reason fields on violations", async () => {
    ...
    const block = events.find(
      (event: any) => event.eventType === "TOOL_CALL_BLOCK",
    );
    expect(block).toBeTruthy();
    expect(block.metadata.event_phase).toBe("block");
    expect(block.metadata.authorization_decision).toBe("BLOCK");
    expect(block.metadata.decision_reason_code).toBe(
      "forbidden_action_static_policy",
    );
  ...
  it("includes hold_token in debug decision logs", async () => {
    ...
    const previousLogLevel =
      typeof process !== "undefined" ? process.env.THOTH_LOG_LEVEL : undefined;
    if (typeof process !== "undefined") {
