...
function wrapObservabilityLike(
  stack: string,
  events: string[],
  next: ToolRun,
): ToolRun {
...
function makeBaseTool(events: string[], called: { value: boolean }): ToolRun {
  return async (input: string) => {
    called.value = true;
...
describe("instrument() compatibility wrappers", () => {
  ...
  const stacks = ["datadog", "langsmith", "opentelemetry", "sentry"] as const;

  it.each(stacks)(
    "coexists with %s-style wrapper on ALLOW for both wrapper orders",
    async (stack) => {
      vi.stubGlobal(
        "fetch",
      ...
      const wrappedBeforeInstrument = wrapObservabilityLike(
        stack,
        eventsOuter,
        baseOuter,
      );
      ...
      thothInner.tools[0].run = wrapObservabilityLike(
        stack,
        eventsInner,
        thothInner.tools[0].run.bind(thothInner.tools[0]),
      );
  ...
  it.each(stacks)(
    "preserves BLOCK behavior with %s-style wrapper on both wrapper orders",
    async (stack) => {
      vi.stubGlobal(
        "fetch",
        vi.fn().mockResolvedValue({
      ...
      await expect(thothOuter.tools[0].run("incident-42")).rejects.toThrow(
      ...
      await expect(thothInner.tools[0].run("incident-42")).rejects.toThrow(
        ThothPolicyViolation,
      );
