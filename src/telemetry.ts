import { EventType, type BehavioralEvent } from "./models.js";
...
const CONTENT: Record<EventType, string> = {
...
const ID_LIST_FIELDS = ["regulatory_regimes", "matched_rule_ids", "matched_control_ids"] as const;
...
function securitySignal(value: string): boolean {
  if (FIXED_SIGNALS.has(value)) return true;
  if (/^(?:moses_action|authz):(allow|block|step_up|modify|defer)$/.test(value)) return true;
  if (/^classification:(read|write|delete|execute|phi|pii|public|internal|confidential|restricted)$/.test(value)) return true;
  if (/^(?:dlp_redactions|delegation_chain_len|defer_depth|retrieval_filter_fields):\d+$/.test(value)) return true;
  if (/^attestation_policy_(?:enabled|required):(?:true|false)$/.test(value)) return true;
  const parts = value.split(":");
  const confidence = (score: string | undefined): boolean =>
    score !== undefined && /^(?:0(?:\.\d+)?|1(?:\.0+)?)$/.test(score);
 ...
 */
export function telemetryEvent(event: BehavioralEvent): BehavioralEvent {
  const source = event.metadata ?? {};
  const metadata: Record<string, unknown> = { telemetry_capture: "minimal" };
  ...
  if (signals) metadata.model_signals = signals.filter(securitySignal);
  ...
  if (event.toolName) metadata.tool_call = { name: event.toolName };
  return {
