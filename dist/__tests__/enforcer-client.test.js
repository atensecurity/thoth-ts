...
function loadGoldenFixture(name) {
    ...
    if (!fixturePath) {
        throw new Error(`unable to locate golden fixture (${fixtureRelativePath}) from ${testDir}`);
    }
...
describe("enforcer-client response mapping", () => {
    ...
    it("continues polling an explicitly unresolved hold", async () => {
        ...
        const decision = await awaitStepUpDecision(buildConfig({ stepUpPollIntervalMs: 1 }), "tok_pending");
        expect(decision.decision).toBe("ALLOW");
    ...
    it("sends action_attestation_id when provided", async () => {
        ...
        vi.stubGlobal("fetch", fetchMock);
        await checkEnforce(buildConfig(), "read:data", "sess_1", ["read:data"], { path: "/tmp/a.txt" }, "trace-1", "attest-1");
        const [, init] = fetchMock.mock.calls[0];
        const body = JSON.parse(String(init.body));
        expect(body.enforcement_trace_id).toBe("trace-1");
        expect(body.action_attestation_id).toBe("attest-1");
    });
    it.each([false, true].flatMap((failOpen) => [
        "policy_backend_unavailable",
        "tenant_compliance_backend_unavailable",
    ].map((reasonCode) => ({ failOpen, reasonCode }))))("preserves HTTP 200 BLOCK for $reasonCode with failOpen=$failOpen", async ({ failOpen, reasonCode }) => {
        const payload = {
            decision: "BLOCK",
        ...
        vi.stubGlobal("fetch", fetchMock);
        const decision = await checkEnforce(buildConfig({ failOpen }), "read:data", "sess_1", ["read:data"]);
        expect(decision.decision).toBe("BLOCK");
    ...
    it("requests a human explanation for BLOCK decisions", async () => {
        ...
        const explanation = await requestHumanExplanation(buildConfig(), {
            decision: "BLOCK",
            violationId: "vio-1",
            decisionReasonCode: "policy_scope_violation",
        }, "write:file", "sess_1", ["write:file"], { path: "/tmp/prod.db" });
