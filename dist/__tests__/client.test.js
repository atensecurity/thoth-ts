...
const mocks = vi.hoisted(() => ({
    ...
    wrapOpenAITools: vi.fn((toolFns) => toolFns),
}));
...
describe("ThothClient", () => {
    ...
    it("delegates instrument() with merged defaults", () => {
        ...
        expect(mocks.instrument).toHaveBeenCalledWith(agent, expect.objectContaining({
            agentId: "agent-1",
            approvedScope: ["search_docs"],
            tenantId: "acme",
            userId: "alice@acme.com",
    ...
    it("delegates anthropic and openai wrappers with merged defaults", () => {
        ...
        expect(mocks.wrapAnthropicTools).toHaveBeenCalledWith(toolFns, expect.objectContaining({
            agentId: "agent-3",
            tenantId: "acme",
            enforcement: "block",
        }));
        expect(mocks.wrapOpenAITools).toHaveBeenCalledWith(toolFns, expect.objectContaining({
            agentId: "agent-4",
            tenantId: "acme",
            enforcement: "block",
        }));
