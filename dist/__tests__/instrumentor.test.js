...
describe("instrument()", () => {
    ...
    it("emits structured PRE/POST tool telemetry metadata", async () => {
        ...
        const enforceBody = JSON.parse(String((enforceCall?.[1] ?? {}).body ?? "{}"));
        ...
        expect(pre.metadata.action_attestation_id).toBe(enforceBody.action_attestation_id);
    ...
    it("raises ThothPolicyViolation on defer decision", async () => {
        ...
        await expect(agent.tools[0].run("arg")).rejects.toThrow(/awaiting additional evidence/i);
    });
    ...
    it("uses THOTH_ENVIRONMENT fallback when environment is omitted", async () => {
        ...
        vi.resetModules();
        const { instrument: instrumentWithEnvFallback } = await import("../instrumentor.js");
        const fetchMock = vi.fn().mockImplementation((url) => {
            if (url.includes("/v1/enforce")) {
                return Promise.resolve({
                    ok: true,
